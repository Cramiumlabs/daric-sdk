...
INT           num_pages;
    ...
    /*
     *  crossbar source modification start
     *  When the vertical scroll bar is pulled up and down too far, there is a blank area, need to pull back,
     *  because the screen refresh rate is too low, resulting in a lag. Therefore, it needs to be modified to
     *  scroll only in the list display list area when the scroll bar is scrolling.
     */
    if (amount < 0)
    {
        child = list->gx_widget_last_child;
        if (child->gx_widget_size.gx_rectangle_bottom + amount <= list->gx_widget_size.gx_rectangle_bottom)
        {
            amount = list->gx_widget_size.gx_rectangle_bottom - child->gx_widget_size.gx_rectangle_bottom;
        }
    }
    if (amount > 0)
    {
        ...
        while (child)
        {
            ...
            {
                if (child->gx_widget_size.gx_rectangle_top + amount >= list->gx_widget_size.gx_rectangle_top)
                {
                    amount = list->gx_widget_size.gx_rectangle_top - child->gx_widget_size.gx_rectangle_top;
                }
                break;
            }
    ...
    /* crossbar source modification end */
