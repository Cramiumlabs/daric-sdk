    ...
    const mbedtls_ssl_ciphersuite_t **selected_ciphersuite_info)
{
    *selected_ciphersuite_info = NULL;
    ...
    /* add by dongshilei
    * Cast psk_hash_alg to (unsigned int) to match the %x format specifier.
    * This is necessary because psa_algorithm_t is typedef'd as long unsigned int,
    * which causes a format mismatch warning (treated as an error with -Werror).
    * While this cast may result in truncation on some systems, it's acceptable
    * for debug output in this context.
    */
    MBEDTLS_SSL_DEBUG_MSG(2, ("No matched ciphersuite, psk_ciphersuite_id=%x, psk_hash_alg=%x",
                              (unsigned) psk_ciphersuite_id, (unsigned int)psk_hash_alg));
}
