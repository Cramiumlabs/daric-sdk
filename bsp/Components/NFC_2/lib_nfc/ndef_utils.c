                              ...
                              bool b_allow_chunks)
{
  ...
  for (count = 0; p_rec < p_end; count++)
  {
    ...
    else
    {
      ...
      // AOSP bug. Actually they must NOT have it...
...
/*******************************************************************************
  ...
  **
  *******************************************************************************/
extern tNDEF_STATUS NDEF_MsgAddChunkRec(tNDEF_CHUNK_TYPE chunk_type, uint8_t *p_msg, uint32_t max_size,
                                   uint32_t *p_cur_size, uint8_t tnf,
                                   uint8_t *p_type, uint8_t type_len,
                                   uint8_t *p_id, uint8_t id_len,
                                   uint8_t *p_payload, uint32_t payload_len)
{
  ...
  switch(chunk_type)
  {
      case NDEF_CHUNCK_INITIAL:
          *p_rec  = tnf | NDEF_CF_MASK;
          break;
      case NDEF_CHUNCK_MIDDLE:
  ...
  if (*p_cur_size == 0)
  {
    *p_rec = *p_rec | NDEF_MB_MASK | NDEF_ME_MASK;
  }
  else
  {
    /* Find the previous last and clear his 'Message End' bit */
