...
struct haptic_nv haptic_nv = {
	.i2c_addr = CONFIG_VIBRATOR_I2C_ADDR,
	.rst_pin = CONFIG_AW_RST_Pin,
	.gain = 0x80,
	.timer_ms_cnt = 0,
	.name = AW_CHIP_NULL,
...
static int get_base_addr(void)
{
	...
	for (i = ram_num * 4; i >= 4; i = i - 4) {
		...
		if ((last_end - g_haptic_nv->ram.base_addr + 1) == g_haptic_nv->aw_fw.len) {
			AW_LOGI("base_addr = 0x%04lx", g_haptic_nv->ram.base_addr);
			return AW_SUCCESS;
		} else {
			ram_num--;
		}
...
static int long_vib_work(uint8_t index, uint8_t gain, uint32_t duration)
{
	...
	if ((duration == 0) || (index == 0)) {
		AW_LOGE("duration = %ld, index = %d, err", duration, index);
		return AW_ERROR;
	}

	AW_LOGI("start duration = %ld, index = %d", duration, index);
...
static int judge_within_cali_range(void)
{
	...
	AW_LOGI("f0_pre = %ld, f0_cali_min = %ld, f0_cali_max = %ld, f0 = %ld",
			g_haptic_nv->info->f0_pre, f0_cali_min, f0_cali_max, g_haptic_nv->f0);

	if (g_haptic_nv->f0 < f0_cali_min) {
		AW_LOGE("lra f0 is too small, lra_f0 = %ld!", g_haptic_nv->f0);
	...
	if (g_haptic_nv->f0 > f0_cali_max) {
		AW_LOGE("lra f0 is too large, lra_f0 = %ld!", g_haptic_nv->f0);
#ifdef AW_MAXIMUM_F0_CALI_DATA
...
static int judge_rtp_load_end(void)
{
	...
	if ((g_haptic_nv->rtp_cnt == haptic_nv_rtp_len) ||
		...
		if (g_haptic_nv->rtp_cnt != haptic_nv_rtp_len)
			...
			AW_LOGI("rtp update complete!,cnt=%ld", g_haptic_nv->rtp_cnt);
...
static int rtp_going(void)
{
	...
	AW_LOGI("cnt = %ld, exit", g_haptic_nv->rtp_cnt);
	return AW_SUCCESS;
}
...
static void rtp_vib_work(uint8_t gain)
{
	...
	AW_LOGI("rtp file size = %ld", haptic_nv_rtp_len);
...
static int parse_chipid(void)
{
	...
	for (cnt = 0; cnt < AW_READ_CHIPID_RETRIES; cnt++) {
		...
		case AW86245_CHIP_ID:
			...
			return 0;
		default:
			AW_LOGI("unsupport device revision (0x%02lX)", reg);
			break;
		}
		haptic_nv_mdelay(2);
	}
